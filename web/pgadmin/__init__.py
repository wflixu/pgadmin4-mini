...
def create_app(app_name=None):
    ...
    if "SCRIPT_NAME" in os.environ and os.environ["SCRIPT_NAME"]:
        app.config.update(dict({"APPLICATION_ROOT": os.environ["SCRIPT_NAME"]}))
    ...
    def before_request():
        ...
        if not config.SERVER_MODE and not current_user.is_authenticated:
            ...
            if user is None:
                ...
                abort(401)
            app.logger.info(f"current_user: {user.email}")
            login_user(user)
        elif (
            config.SERVER_MODE
        ...
        if (
            ...
            and app.PGADMIN_EXTERNAL_AUTH_SOURCE not in [KERBEROS, OAUTH2, WEBSERVER]
