...
function startDesktopMode() {
  ...
  document.getElementById("loader-text-status").innerHTML =
    "Starting pgAdmin 4...";
  ...
  misc.writeServerLog(
    'Runtime Config File: "' + path.resolve(misc.getRunTimeConfigFile()) + '"'
  );
  misc.writeServerLog(
    'pgAdmin Config File: "' + path.resolve(configFile) + '"'
  );
  ...
  Object.keys(process.env).forEach(function (key) {
    ...
    if (platform() === "win32" && (key === "PATH" || key == "Path")) {
      let _libpq_path = path.join(
        path.dirname(path.dirname(path.resolve(pgadminFile))),
        "runtime"
      );
  ...
  console.log("---start server ----", pythonPath, path.resolve(pgadminFile));
  ...
  misc.writeServerLog(
    "Total spawn time to start the pgAdmin4 server: " +
      (spawnEndTime - spawnStartTime) / 1000 +
      " Sec"
  );
  ...
  function pingServer() {
    console.warn(serverCheckUrl);
    return axios.get(serverCheckUrl);
  }

  let connectionTimeout =
    misc.ConfigureStore.get("connectionTimeout", 90) * 1000;
  ...
  let intervalID = setInterval(function () {
    ...
    pingServer()
      .then(() => {
        ...
        misc.writeServerLog("------------------------------------------");
        misc.writeServerLog(
          "Total time taken to ping pgAdmin4 server: " +
            (appEndTime - pingStartTime) / 1000 +
            " Sec"
        );
        misc.writeServerLog("------------------------------------------");
        misc.writeServerLog(
          "Total launch time of pgAdmin4: " +
            (appEndTime - appStartTime) / 1000 +
            " Sec"
        );
        misc.writeServerLog("------------------------------------------");
        launchPgAdminWindow();
  ...
  nw.Window.open(
    ...
    (pgadminWindow) => {
      ...
      pgadminWindow.on("new-win-policy", function (frame, url, policy) {
        if (!frame) {
          let openDocsInBrowser = misc.ConfigureStore.get(
            "openDocsInBrowser",
            true
          );
          let isDocURL = false;
      ...
      pgadminWindow.on("loaded", function () {
        ...
        try {
          ...
          let addMenuInterval = setInterval(() => {
            if (
              pgadminWindow?.window?.pgAdmin?.Browser?.Events &&
              pgadminWindow?.window?.pgAdmin?.Browser?.MainMenus?.length > 0
            ) {
              pgadminWindow.window.pgAdmin.Browser.Events.on(
                "pgadmin:nw-enable-disable-menu-items",
                enableDisableMenuItem
              );
              pgadminWindow.window.pgAdmin.Browser.Events.on(
                "pgadmin:nw-refresh-menu-item",
                refreshMenuItems
              );
              pgadminWindow.window.pgAdmin.Browser.Events.on(
                "pgadmin:nw-update-checked-menu-item",
                updateCheckedMenuItem
              );
              pgadminWindow.window.pgAdmin.Browser.Events.on(
                "pgadmin:nw-set-new-window-open-size",
                setNewWindowSize
              );
    ...
    misc
      ...
      .catch((errCode) => {
        if (errCode === "EADDRINUSE") {
          alert(
            "The port specified is already in use. Please enter a free port number."
          );
        } else {
          alert(errCode);
        }
...
function getRuntimeMenu() {
  ...
  let rtmenudt =
    pgAdminMainScreen.window.pgAdmin.Browser.RUNTIME_MENUS_OPTIONS["runtime"];
  let runtimeSubMenus =
    pgAdminMainScreen.window.pgAdmin.Browser.RUNTIME_MENUS_OPTIONS["runtime"][
      "submenus"
    ];
          ...
          item.menu_items.forEach((subItem) => {
            submenu.append(
              new gui.MenuItem({
                ...
                type: [true, false].includes(subItem.checked)
                  ? "checkbox"
                  : "normal",
