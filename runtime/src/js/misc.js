...
const getLocalAppDataPath = () => {
  ...
  switch (platform()) {
    case "win32":
      ...
      break;
    case "darwin":
      localAppDataPath = path.join(homedir(), "logs", "pgadmin");
      break;
    case "linux":
...
const serverLogFile = path.join(
  getLocalAppDataPath(),
  "pgadmin4." + currentTime.toString() + ".log"
);
    ...
    pgAdminWindowObject.cookies.getAll({}, function (cookies) {
      try {
        cookies.forEach(function (cookie) {
          pgAdminWindowObject.cookies.remove({
...
const setZoomEvents = () => {
  ...
  toggleFullScreenShortcut.on("failed", function (msg) {
    let errMsg =
      "Failed to register toggle full screen shortcut with error: " + msg;
    console.warn(errMsg);
  });
};
...
const toggleFullScreen = () => {
  if (pgAdminWindowObject != null) {
    ...
    let menu_label = pgAdminWindowObject.window.document.querySelector(
      "#mnu_toggle_fullscreen_runtime span"
    ).innerHTML;
    if (menu_label.indexOf("Enter Full Screen") >= 0) {
      pgAdminWindowObject.window.document.querySelector(
        "#mnu_toggle_fullscreen_runtime span"
      ).innerHTML = menu_label.replace("Enter", "Exit");
    } else if (menu_label.indexOf("Exit Full Screen") >= 0) {
      pgAdminWindowObject.window.document.querySelector(
        "#mnu_toggle_fullscreen_runtime span"
      ).innerHTML = menu_label.replace("Exit", "Enter");
    }
  }
};
...
let ConfigureStore = {
  ...
  saveConfig: function () {
    fs.writeFileSync(this.fileName, JSON.stringify(this.jsonData, null, 4), {
      flag: "w",
    });
  },
